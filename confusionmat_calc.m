function [confusion_mat Error_mat] = confusionmat_calc(label,Yts,ct,ts_samples)
...
Error_mat = zeros(1,4);
confusion_mat = zeros(3,3); % count of number of samples misclassified
for i=1:ts_samples
    if ((label(i)~= Yts(i))&& (label(i)==1))
        if Yts(i)==2
            confusion_mat(1,2) = confusion_mat(1,2) + 1;
        else
            confusion_mat(1,3) = confusion_mat(1,3) + 1;
        end
    elseif ((label(i)~= Yts(i))&& (label(i)==2))
         if Yts(i)==1
            confusion_mat(2,1) = confusion_mat(2,1) + 1;
        else
            confusion_mat(2,3) = confusion_mat(2,3) + 1;
        end
    elseif ((label(i)~= Yts(i))&& (label(i)==3))
        if Yts(i)==1
